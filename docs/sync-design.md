...
- **新设备接入的密钥分发**（2026-08-21 补）：保存远端配置时若本机尚无密钥，
  先 LIST 同步前缀探测远端：
  - 远端为空 → 本机是首个设备，生成新钥（仅返回一次供抄录）；
  - 远端已有数据 → **不生成新钥**（自动生成会导致各设备密钥不一致、互相解不开
    对方备份甚至分叉覆盖），置 `sync.needs_key_import` 标记，配置页黄条警告并
    阻止启用，直到用户从旧设备导入密钥；
  - 探测失败（网络/凭证错）→ 保存整体报错——盲存无法确认远端状态，正是危险路径。
  导入密钥时若远端已有数据，尝试解密一份备份验证匹配；不匹配则拒绝导入
  （错钥静默跳过 dump 同样会分叉）。远端为空或不可达时不拦截（离线可导入）。
