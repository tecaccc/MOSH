...
- **AI 助手**：
  - **提供商与模型实体管理**：预置服务商一键添加（官方地址预填），支持任意 OpenAI 兼容自定义端点；每家可同步/手动管理多个模型（能力标签、置顶、启停、隐藏），全局默认模型 + 聊天框内随时切换（搜索/分组/能力点标的选择器）；模型/提供商图标自动识别（@lobehub/icons，未识别回退首字母圆标）
  - 自然语言操作待办与日程（创建 / 查询 / 修改 / 删除），流式输出；回复气泡标注所用模型
