...
## 待办完成时间点 + GitHub Actions 自动发布 + 应用内自动更新
...
## 补录归档：AI 修改日程与周期提醒（此前已实现，本次验证补记）
...
## AI 能力增强：Skills + MCP + 聊天输入区改版 + 会话管理
...
| 【美化】自绘弹窗替换浏览器默认    | 新增 `state/dialog.ts`（zustand：Promise 化 `confirm(opts)→Promise<boolean>` / `prompt(opts)→Promise<string | null>`）、`DialogHost.tsx` 全局宿主挂载于 App 根部（Enter 确认、Esc/遮罩取消、自动聚焦，danger=true 时红色删除风格，样式在 `DialogHost.module.css`）；替换原有原生 confirm/prompt 调用（TodoItem 删除/子任务、TodoEditor 取消确认、ChatPanel 删除会话、AgentToolsSettings 删除确认等），深浅主题与既有风格一致。（注：原文此行在 TODO-List.md 中被截断，本句依据代码实际补全） |
